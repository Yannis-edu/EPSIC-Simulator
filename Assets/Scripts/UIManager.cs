...
public class UIManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (VD.isActive) //If there is a dialogue active
        {
            ...
            if (!data.pausedAction && !animatingText && data.isPlayer && !useNavigation)
            {
                ...
                {
                    maxPlayerChoices[i].transform.GetChild(0).GetComponent<Text>().color = Color.black;
                    if (i == data.commentIndex) maxPlayerChoices[i].transform.GetChild(0).GetComponent<Text>().color = Color.red;
                }
            }
    ...
    void UpdateUI(VD.NodeData data)
    {
        ...
        NPC_Text.text = "";
        foreach (Button b in maxPlayerChoices) { b.transform.GetChild(0).GetComponent<Text>().text = ""; b.transform.GetChild(0).GetComponent<Text>().color = Color.black; }
